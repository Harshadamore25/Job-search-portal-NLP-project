...
df=pd.read_csv("scrapped_data.csv",index_col=0, encoding = "utf-8")
df_new=df.copy()
cols=['Ratings','Reviews','URL']
...
df_new['Title']= label_encoder.fit_transform(df_new['Title'])  
df_new['Company']= label_encoder.fit_transform(df_new['Company'])
df_new['Experience']= label_encoder.fit_transform(df_new['Experience'])
df_new['Job_Post_History']= label_encoder.fit_transform(df_new['Job_Post_History'])
df_new['Location']= label_encoder.fit_transform(df_new['Location'])
df_new['Salary']= label_encoder.fit_transform(df_new['Salary'])
df_new['Skills']= label_encoder.fit_transform(df_new['Skills'])




def my_widget(key):
    st.subheader('Search based on specific features')
    return st.button("Click here " + key)

# This works in the main area
clicked = my_widget("to refresh selection below")


#def expan(click1,click2,click3,click4,click5,click6,click7):
#expand = st.expander(label='Expand to select criterion wise')
#with expand:
#    'Search for top results based on'
#    click1 = st.button('Titles')
#    click2 = st.button('Companies')
#    click3 = st.button('Experiences')
#    click4 = st.button('Date of Job Posting')
#    click5 = st.button('Locations')
#    click6 = st.button('Salaries')
#    click7 = st.button('Skills')


if st.button('Job titles'):
#if expan == click1:
     #Ttitlewise
    st.title('Top 25 Titles')
    ...
    st.pyplot(fig1)
elif st.button('Company name'):
#elif expan == click2:
    #Based on Company
    st.title('Top 15 Companies')
    ...
    st.pyplot(fig2)
elif st.button('Years of experience'):
#elif expan == click3:
    #Based on Experience
    ...
    st.pyplot(fig3)
elif st.button('Job Posting date'):
#elif expan == click4:
    #Job posted on
    st.title('Dates on which Jobs were posted')
    ...
    #st.pyplot(bar1)
    st.set_option('deprecation.showPyplotGlobalUse', False)
elif st.button('Range of salaries'):
#elif expan == click6:
    #Based on Salary
    st.title('Salary Ranges')
    ...
    st.pyplot(fig6)
    st.set_option('deprecation.showPyplotGlobalUse', False)
elif st.button('Top skills'):
#elif expan == click7:
    #Based on Skills
    st.title('Top 25 Skills')
    ...
    st.pyplot(fig7)
elif st.button('Relation between features'):
#elif expan == click8:
    st.title('Correlation between all variables')
    d=df_new.corr()
    d

    st.subheader('Heatmap')
    fig8=plt.figure(figsize = (5,5))
    sns.heatmap(df_new.corr(), annot=False)
    st.pyplot(fig8)

    st.title('Correlation-plot between all variables')

    st.subheader('Pairplot')
    #fig9=plt.figure()
    fig9=sns.pairplot(df_new)
    st.pyplot(fig9)

    st.subheader('Pairplot w.r.t. Title')
    #fig10=plt.figure()
    fig10=sns.pairplot(df_new, hue="Title")
    st.pyplot(fig10)
else:
    st.write('Please select your search criterion from above')
